...
;; MATH STUFF
(def digits (map read-string (remove #{"."} (map str (str/trim (str/trim-newline (slurp "20k-phi.txt")))))))
(def mini-digits (take 400 digits))
...
(defn play-harmony-segment
  ...
  (let [list list
    ...
    (when dgt
      (at t
          (sampled-piano (+ pitch-adjust (get-scale-val dgt har-scale)) 0.25)
          (when (upcoming-double? list)
            (println-str dgt)
            (sampled-piano (+ 20 (mod (+ pitch-adjust (get-scale-val dgt har-scale)) 24)) 0.55)
            (sampled-piano (+ 32 (mod (+ pitch-adjust (get-scale-val dgt har-scale)) 24)) 0.60))
        (apply-by t-next #'play-harmony-segment [t-next base-ms (rest list) har-scale (get-pitch-adjust-for-chords pitch-adjust dgt har-scale 108 20)])))))
...
(defn play-the-thing
  ...
  (let [coll coll
        ...
        harm-ms (int (/ mel-length (* 8 (count (first coll)))))
        t-next (+ t mel-length)]
    (when (not (empty? (rest coll)))
      (at t
          (play-harmony-segment t
                                harm-ms
                                (take (* 8 (count (first coll))) (cycle (first coll)))
                                har-minor-1
                                har-pitch-adjust)
          (play-melody-segment t
                               ...
                               (first coll)
                               mel-minor
                               (get-pitch-adjust-for-segment (first coll) mel-major mel-pitch-adjust 96 48 9))
...
(play-the-thing (now) 650 mini-n-sub-by-triples 48 32)

(recording-start "~/Desktop/400-phi.wav")
